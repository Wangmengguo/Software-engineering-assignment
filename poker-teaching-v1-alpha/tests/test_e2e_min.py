...
from poker_core.state_hu import start_session, start_hand, apply_action, settle_if_needed

def total(gs): 
    # 正确计算：玩家筹码 + 底池 + 当前街投资
    return gs.players[0].stack + gs.players[1].stack + gs.pot + gs.players[0].invested_street + gs.players[1].invested_street

def apply_and_check(gs, action, amount=None, total0=None):
    if total0 is None: total0 = total(gs)
