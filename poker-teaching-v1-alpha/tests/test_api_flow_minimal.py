...
def test_minimal_flow():
    ...
    r = c.post("/api/v1/hand/start", data=json.dumps({"session_id": sid}), content_type="application/json")
    assert r.status_code == 200
    hid = r.json()["hand_id"]
    ...
    r = c.post(f"/api/v1/hand/act/{hid}", data=json.dumps({"action": act}), content_type="application/json")
    assert r.status_code == 200
...
def test_act_returns_outcome_when_hand_over(client):
    # 1) start session
    sid = client.post("/api/v1/session/start", data='{}', content_type="application/json").json()["session_id"]
    # 2) start hand with fixed seed
    hid = client.post("/api/v1/hand/start", data=json.dumps({"session_id":sid,"seed":42}), content_type="application/json").json()["hand_id"]
    ...
    for _ in range(30):
        ...
        r = client.post(f"/api/v1/hand/act/{hid}", data=json.dumps({"action": la[0]}), content_type="application/json").json()
        if r["hand_over"]:
            assert "outcome" in r
...
def test_replay_has_winner_and_best5(client):
    ...
    for _ in range(30):  # 最多30步防止无限循环
        ...
        # 执行动作
        act_response = client.post(f"/api/v1/hand/act/{hid}", 
    ...
    assert replay_response.status_code == 200, f"Replay API should return 200, got {replay_response.status_code}"
