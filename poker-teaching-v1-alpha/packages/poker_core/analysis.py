...
def _hole_features(cards: List[str]) -> Dict[str, Any]:
    ...
    return {
        "pair": pair, "suited": suited, "gap": gap,
        "high": hi, "low": lo, "has_ace": has_ace,
...
def _derive_tags(feat: Dict[str, Any]) -> Tuple[Set[str], str]:
    ...
    # 兜底弱牌标签（可按你原规则继续扩展）
    if "suited_broadway" not in tags and "pair" not in tags and "Ax_suited" not in tags and "broadway_offsuit" not in tags:
        tags.add("weak")
